...
export default class TodoList extends React.Component {
  ...
  /*  AsyncStorage.getItem('todolist').then(response =>
    ...
    /*
    const databasetodolist=firebase.database().ref('todolist');
    databasetodolist.on('value',snap => {
      var elenco = [];
         snap.forEach(child => {
        elenco.push({
         text: child.val().text,
    ...
    if(currentUID){
      const path=".user/"+currentUID+"/tolist";
      firebase.database().ref(path).on('value',snap =>{
     this.setState({todolist:Object.values(snap.val() ? snap.val() : [])})
    });
    }
  }
  ...
  _add = todo => {
   ...
   // this._update(newTodolist);
    ...
    const currentUID=firebase.auth().currentUser.uid;
    const path="/user/"+currentUID+"/tolist";
      const databasetodolist=firebase.database().ref(path);
 databasetodolist.push({todolist});
  };
